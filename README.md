...
| OS | 실행 파일 경로 |
|----|----------------|
| macOS | `release/mac-universal/포켓몬키우기.app` |
| Windows (압축X) | `release\win-unpacked\poketmon-idle.exe` |
| Windows (인스톨러) | `release\포켓몬키우기 Setup 1.0.0.exe` |

- macOS는 Finder에서 `.app`을 더블클릭하거나 `open release/mac-universal/포켓몬키우기.app`
- Windows는 `.exe`를 더블클릭

> **macOS 보안 경고**: 서명되지 않은 빌드라 처음 실행 시 "확인되지 않은 개발자" 경고가 뜰 수 있다.
> `시스템 설정 → 개인정보 보호 및 보안`에서 "확인 없이 열기" 클릭 또는 `.app` 우클릭 → 열기 → 열기.
...
```bash
xattr -cr release/mac-universal/포켓몬키우기.app
```
실행 후 다시 열기.
